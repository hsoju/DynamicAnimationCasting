...
namespace Loki {
    ...
    public:
		...
		static inline std::unordered_map<std::string, std::vector<Loki::AnimationCasting::Cast*>> _additions = {};
		static inline std::unordered_map<std::string, std::vector<RE::SpellItem*>*> _additionsCache = {};

		static void ReplaceSpells(const std::string& a_uuid, const std::vector<RE::SpellItem*>& a_newSpells);

		//static void LoadAdditions();
